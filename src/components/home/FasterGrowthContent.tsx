...
export default function FasterGrowthContent({
  ...
  const {bg, text1, text2} = useMemo(() => colors[type], [type]);
        ...
        <div className="sm:flex sm:flex-col relative w-full sm:w-1/2 text-left py-6">
          ...
          <div className="py-6">
            <button
              ...
              className={`${bg} ${type === "buyer" ? "w-full lg:w-auto": ""} flex items-center justify-center text-xl h-16 px-5 lg:mr-6 font-bold tracking-normal text-white transition duration-200 rounded-full focus:shadow-outline focus:outline-none`}
            >
              {buttonText} 
      ...
      <Fade right duration={1000} delay={500} distance="30px">
        <div className="relative items-center sm:w-1/2 justify-center m-auto sm:rounded-none">
          ...
          <div className="-z-10 absolute -top-7 -right-1/3 lg:-top-14 lg:-right-1/4">
            <Leaves className={`${text1} w-[274px] h-[280px] lg:w-auto lg:h-auto`} />
          </div>
        </div>
      </Fade>
    </div>
